...
import {
  Alert, Button, Divider, Flex, InputNumber, Modal, Segmented, Select, Spin, Table, Tag, Tooltip, Typography,
} from 'antd'
...
export default function FiscalYearsPage() {
  ...
  // Yearly form
  const [fullYear, setFullYear] = useState(currentYear)
  ...
  // Yearly period preview
  const yearlyName      = `سنة ${fullYear}`
  const yearlyStartDate = `${fullYear}-01-01`
  const yearlyEndDate   = `${fullYear}-12-31`
    ...
    try {
      if (periodMode === 'yearly') {
        await api.post('/api/fiscal-years', {
          name:       yearlyName,
          start_date: yearlyStartDate,
          end_date:   yearlyEndDate,
        })
    ...
    <div>
      ...
      <Flex justify="space-between" align="center" style={{ marginBottom: 24 }}>
        <div>
          ...
          <Text type="secondary">إدارة الفترات المالية الشهرية والسنوية</Text>
        </div>
        <Flex gap={8} align="center">
          <HelpButton title="دليل استخدام الفترات المالية">
            <Flex vertical gap={16}>
              <div><Title level={5}>ما هي الفترة المالية؟</Title>
                <Text>الفترة المالية هي النطاق الزمني الذي يُسجَّل فيه النشاط المحاسبي — يمكن أن تكون شهرية أو سنوية.</Text></div>
              <div><Title level={5}>إنشاء فترة جديدة</Title>
                <Text>اضغط "فترة جديدة"، واختر النوع (شهرية أو سنوية). للفترة الشهرية اختر الشهر والسنة، ويمكن أيضاً إنشاء جميع أشهر السنة دفعة واحدة. للفترة السنوية يكفي اختيار السنة، وستُنشأ فترة واحدة تغطي 1 يناير حتى 31 ديسمبر منها.</Text></div>
              ...
              <div><Title level={5}>كيف يعمل ترحيل الأرصدة بين الفترات؟</Title>
                <Text>
                  عند الإغلاق: تُصفَّر حسابات الإيرادات والمصروفات وينتقل صافي الربح أو الخسارة إلى حساب الأرباح المحتجزة — هذه الحسابات تبدأ كل فترة من الصفر دائماً.
                  <br /><br />
                  حسابات الأصول والخصوم وحقوق الملكية لا تُصفَّر أبداً — الرصيد الافتتاحي لأي فترة جديدة = (الرصيد الافتتاحي الخاص بالفترة المغلقة السابقة لها) + (مجموع حركة كل القيود المرحَّلة حتى نهاية تلك الفترة، من أي وقت سابق).
                  <br /><br />
                  يحدث هذا الحساب تلقائياً في أي من اللحظتين: عند إغلاق فترة إن كانت الفترة التالية موجودة مسبقاً، أو عند إنشاء فترة جديدة إن كانت الفترة السابقة لها مغلقة بالفعل — أيهما أسبق.
                </Text></div>
            </Flex>
          </HelpButton>
        ...
        footer={
          <Flex justify="flex-end" gap={8}>
            ...
            <Button type="primary" onClick={handleCreate} disabled={createDisabled} icon={creating ? <Spin size="small" /> : <Plus size={16} />}>
              {periodMode === 'monthly' && monthMonth === 0 ? 'إنشاء 12 شهراً' : 'إنشاء'}
            </Button>
          </Flex>
        }
      >
        <Flex vertical gap={16} style={{ paddingTop: 8 }}>
          <Segmented
            block
            value={periodMode}
            onChange={v => setPeriodMode(v as 'monthly' | 'yearly')}
            options={[
          ...
          {periodMode === 'monthly' ? (
            ...
            <InputNumber min={2000} max={2100} value={fullYear} onChange={v => setFullYear(Number(v))} style={{ width: 110 }} />
          )}
          ...
          <div style={{ padding: 12, background: 'var(--ant-color-fill-alter)', borderRadius: 6, border: '1px solid var(--ant-color-border-secondary)' }}>
            ...
            {periodMode === 'monthly' ? (
              ...
              <>
                <div style={{ fontWeight: 600, marginTop: 2 }}>{yearlyName}</div>
                <Text type="secondary" style={{ direction: 'ltr', display: 'block', fontSize: 12 }}>
                  {yearlyStartDate} — {yearlyEndDate}
                </Text>
              </>
            )}
          </div>
          ...
          <Alert
            type="info"
            showIcon
            message="الرصيد الافتتاحي يُحسَب تلقائياً"
            description="إن كانت هناك فترة سابقة مغلقة تنتهي قبل بداية هذه الفترة، سيُحسَب رصيدها الافتتاحي فور الإنشاء = (الرصيد الافتتاحي لتلك الفترة) + (حركة كل القيود المرحَّلة حتى نهايتها). حسابات الإيراد والمصروف تبدأ دائماً من صفر ولا تُرحَّل."
          />
        </Flex>
      </Modal>
        ...
        {closeTarget && (
          <Flex vertical gap={16} style={{ paddingTop: 8 }}>
            ...
            <Alert
              type="info"
              showIcon
              message="ماذا يحدث عند الإغلاق؟"
              description={
                <>
                  يُنشَأ قيد إقفال مرحَّل يُصفّر حسابات الإيرادات والمصروفات، وينتقل صافي الربح/الخسارة إلى حساب الأرباح المحتجزة أعلاه.
                  <br />
                  حسابات الأصول والخصوم وحقوق الملكية لا تتأثر بهذا التصفير — أرصدتها تستمر. إن كانت الفترة التالية موجودة بالفعل، سيُحسَب رصيدها الافتتاحي الآن تلقائياً؛ وإلا سيحدث ذلك لاحقاً عند إنشائها.
                </>
              }
            />
          </Flex>
